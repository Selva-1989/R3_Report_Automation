...
public class WebScrap_ORG_PV_Phone_OtherORG {
	WebDriver driver;
	String Url = null;
	Map<String,String> provPhoneMap = null;
	Map<String,String> provPhoneMapDummy = null;
	WebDriverWait wait;
	public WebScrap_ORG_PV_Phone_OtherORG(WebDriver TestBaseClassDriver) {
	...
	public void check_ORG_PV_Phone_Accurate_OtherORG(String clonedR3File, int executingRowIndex, String phoneValidationPriority,
		...
		try {
			...
			String searchString2 = providerName+" "+credentials+" "+state;
			...
			for(String eachSearchString : searchStringList) {
				...
				List<String> keywords = Arrays.asList("facebook", "linkedin", "Healthgrades");
				for (WebElement webElement : linkHeaderList) {
					...
					for (String keyword : keywords) {
						if (!eachSearchLinkHeaderWithoutSpace.startsWith(keyword)) {
